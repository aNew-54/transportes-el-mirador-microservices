...
@SpringBootTest(classes = FacturacionClientStubTest.Config.class)
class FacturacionClientStubTest {
    ...
    static {
        ...
        SERVIDOR.start();
        System.setProperty("clients.facturacion.url", "http://127.0.0.1:" + SERVIDOR.getAddress().getPort());
    ...
    static void apagar() {
        SERVIDOR.stop(0);
        System.clearProperty("clients.facturacion.url");
    ...
    void enviaCabeceraIdempotenciaCorrectamenteYRespondeBien() {
        cuerpo = "{}"; 
        expectedIdempotencyKey = "VIA-1:ORD-1:conformidad";
        
        ConformidadPeticion peticion = new ConformidadPeticion("VIA-1", "ORD-1", "FIRMADA", OffsetDateTime.now(), List.of(), List.of());
        
        assertThatCode(() -> pasarela.registrarConformidad(peticion)).doesNotThrowAnyException();
    }
    ...
    @Configuration
    @EnableFeignClients(clients = FacturacionClient.class)
    ...
    static class Config {
        ...
        FacturacionGateway facturacionGateway(FacturacionClient cliente) {
            return new FacturacionGateway(cliente);
        }
    }
}
