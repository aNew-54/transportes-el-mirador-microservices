...
public record Dinero(BigDecimal monto, String codigoMoneda) {
    ...
    public Dinero porcentaje(BigDecimal porcentaje) {
        ...
        if (porcentaje.compareTo(BigDecimal.ZERO) < 0) {
            throw new IllegalArgumentException("El porcentaje no puede ser negativo: " + porcentaje);
        }
        BigDecimal resultado = this.monto
            .multiply(porcentaje)
            .divide(BigDecimal.valueOf(100), 2, RoundingMode.HALF_UP);
        return new Dinero(resultado, this.codigoMoneda);
    }
    ...
    private void validarMismaMoneda(Dinero otro) {
        if (otro == null) {
            throw new IllegalArgumentException("El dinero a operar o comparar es obligatorio");
        }
