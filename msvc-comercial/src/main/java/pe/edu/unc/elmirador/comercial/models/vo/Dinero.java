...
public record Dinero(BigDecimal monto, String codigoMoneda) {
    ...
    public Dinero multiplicarPor(BigDecimal factor) {
        if (factor == null) {
            throw new IllegalArgumentException("El factor de multiplicacion es obligatorio");
        }
        ...
        return new Dinero(nuevoMonto, this.codigoMoneda);
    }

    public Dinero mitad() {
        BigDecimal nuevoMonto = this.monto.divide(BigDecimal.valueOf(2), 2, RoundingMode.HALF_UP);
        return new Dinero(nuevoMonto, this.codigoMoneda);
    }
